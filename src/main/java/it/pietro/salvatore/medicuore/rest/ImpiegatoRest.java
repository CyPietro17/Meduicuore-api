...
import it.pietro.salvatore.medicuore.service.ImpiegatoService;
...
public class ImpiegatoRest {
  ...
  public ResponseEntity<ImpiegatoResponseDto> modificaImp(@RequestParam(name = "id") Long id,
                                                          @RequestBody ImpiegatoRequestDto requestDto) {
    ImpiegatoResponseDto responseDto = impiegatoService.response(impiegatoService.update(id, requestDto));
    if (responseDto.getN_id() != -1L) {
      return new ResponseEntity<>(responseDto, HttpStatus.OK);
    }
  ...
  @PostMapping(value = "/filter/assunti")
  @Operation(description = "Filtro impiegati assunti")
  public ResponseEntity<List<ImpiegatoResponseDto>> filterAss(@RequestBody ImpiegatoRequestDto requestDto) {
    ImpiegatoResponseDto responseDto;
    List<ImpiegatoResponseDto> listaFiltrata = new ArrayList<>();
    for (Impiegato impiegato : impiegatoService.filterAssunti(requestDto)) {
      responseDto = impiegatoService.response(impiegato);
      listaFiltrata.add(responseDto);
    }
    return new ResponseEntity<>(listaFiltrata, HttpStatus.OK);
  }

  @PostMapping(value = "/filter/dimessi")
  @Operation(description = "Filtro impiegati dimessi")
  public ResponseEntity<List<ImpiegatoResponseDto>> filterDimess(@RequestBody ImpiegatoRequestDto requestDto) {
    ImpiegatoResponseDto responseDto;
    List<ImpiegatoResponseDto> listaFiltrata = new ArrayList<>();
    for (Impiegato impiegato : impiegatoService.filterDimessi(requestDto)) {
      responseDto = impiegatoService.response(impiegato);
      listaFiltrata.add(responseDto);
    }
    return new ResponseEntity<>(listaFiltrata, HttpStatus.OK);
